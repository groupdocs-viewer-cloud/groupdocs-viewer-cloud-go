...
func (c *APIClient) decode(v interface{}, b []byte, contentType string) (err error) {
	...
	case strings.Contains(contentType, "application/octet-stream"):
		// Ensure that `v` is of type `**os.File` (pointer to pointer to os.File)
		filePtr, ok := v.(**os.File)
		if !ok {
			return errors.New("expected **os.File for octet-stream content type")
		}

		// Create a temporary file to store the byte stream
		...
		// Close the temp file and reopen for reading
		if err := tmpFile.Close(); err != nil {
			return err
		}

		// Open the temp file for reading and assign it to the pointer
		file, err := os.Open(tmpFile.Name())
		if err != nil {
			return err
		}

		// Set the value of the dereferenced pointer
		*filePtr = file
		return nil
	default:
